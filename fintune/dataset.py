...
from data_utils import Tokenizer, GraphTokenizer, SMILESTokenizer


class SmilesDataset():
    def __init__(self, path: str, max_len: int, mask_alpha: int, mask_beta: int,
                 perm_size: int, batch_size: int, stage: str = "pretrain",
                 tokenizer: type(Tokenizer) = SMILESTokenizer(special_tokens=['{bos}', '{eos}']),
                 **kwargs) -> None:
        self.df = None
        ...
        if stage == 'finetune':
            label_df = self.df.iloc[:, 1:]
            self.target = []
            self.label_mask = []
            for i in range(len(label_df)):
                data = np.array(label_df.loc[i].tolist())
                mask = np.isnan(data)
                data[mask] = -1
                label_mask = (1-mask).astype(np.float32)
                self.target.append(data)
                self.label_mask.append(label_mask)
            self.num_labels = len(self.df.columns) - 1 
    ...
    def __getitem__(self, idx: int):
        if self.stage == "pretrain":
            ...
            return self.dataset[idx], self.target[idx], self.label_mask[idx]
    ...
    def get_input_data_slice_map(self, smiles):
        ...
        atoms, input_mask = self.__pad_data(tokenized_data, self.max_len)
        if self.stage == "pretrain":
            ...
            target_mask = feature['target_mask']
            label = feature["target"]  
            return input_k, perm_mask, target_mapping, target_mask, input_q, label
        elif self.stage == "finetune":
            input_k = atoms[:-1].astype(np.int32)
            return input_k, input_mask
        elif self.stage == "predict":
            input_k = atoms[:-1]
            return input_k
    ...
    def get_data(self, rank_size=1, rank_id=0):
        ...
        if self.stage == "pretrain":
            ...
            output_columns = ['input_k', 'perm_mask', 'target_mapping', 'target_mask', 'input_q', 'label']
            dataset = ds.GeneratorDataset(source=self, shuffle=True,
                                          column_names=input_columns,
                                          num_shards=rank_size, shard_id=rank_id)
            dataset = dataset.map(operations=map_func, input_columns=input_columns, output_columns=output_columns, num_parallel_workers=24)
            dataset = dataset.project(columns=['input_k', 'label', 'perm_mask', 'target_mapping', 'target_mask', 'input_q'])        
            dataset = dataset.batch(self.batch_size, num_parallel_workers=24)
            return dataset

        elif self.stage == "finetune":
            input_columns = ['smiles']
            output_columns = ['input_k', 'input_mask']
            dataset = ds.GeneratorDataset(source=self, shuffle=False,
                                          column_names=['smiles', 'label', 'label_mask'])
            dataset = dataset.map(operations=map_func, input_columns=input_columns, output_columns=output_columns)
            dataset = dataset.map(operations=type_cast_op, input_columns=['label'])
            dataset = dataset.project(columns=['input_k', 'label', 'label_mask', 'input_mask'])
            dataset = dataset.batch(self.batch_size)
            return dataset
        
        elif self.stage == "predict":
            input_columns = ['smiles']
            output_columns = ['input_k']
            dataset = ds.GeneratorDataset(source=self, shuffle=True,
                                          column_names=['smiles'])
            dataset = dataset.map(operations=map_func, input_columns=input_columns, output_columns=output_columns)
            dataset = dataset.project(columns=['input_k'])
            dataset = dataset.batch(self.batch_size)
            return dataset

    @staticmethod
    def __pad_data(data: List[int], max_len: int) -> np.ndarray:
        input_mask = np.zeros_like(data)
        paded_input_mask = np.pad(input_mask, (0, max_len - len(input_mask) - 1), 'constant', constant_values=1)
        padded_atoms = np.pad(data, (0, max_len - len(data)), 'constant', constant_values=0)
        return padded_atoms, paded_input_mask
