...
class XLNetLayer(nn.Cell):
    def __init__(self, n_token, n_layer, n_head, d_head, d_inner, d_model, dropout, dropatt,
                 attn_type, bi_data, clamp_len, same_length):
        ...
        self.r_w_bias = Parameter(initializer(HeUniform(), [self.n_layer, self.n_head, self.d_head], dtype=ms.float32))
        self.r_r_bias = Parameter(initializer(HeUniform(), [self.n_layer, self.n_head, self.d_head], dtype=ms.float32))

        self.mask_emb = Parameter(initializer(HeUniform(), [1, 1, self.d_model], dtype=ms.float32))

        # post-attention projection (back to `d_model`)
        self.proj_o = Parameter(initializer(HeUniform(), [self.d_model, self.n_head, self.d_head], dtype=ms.float32))

        # Project hidden states to a specific head with a 4D-shape.
        self.q_proj_weight = Parameter(initializer(HeUniform(), [self.d_model, self.n_head, self.d_head], dtype=ms.float32))
        self.k_proj_weight = Parameter(initializer(HeUniform(), [self.d_model, self.n_head, self.d_head], dtype=ms.float32))
        self.v_proj_weight = Parameter(initializer(HeUniform(), [self.d_model, self.n_head, self.d_head], dtype=ms.float32))
        self.r_proj_weight = Parameter(initializer(HeUniform(), [self.d_model, self.n_head, self.d_head], dtype=ms.float32))
    ...
    def head_projection(self, h, name):
        ...
        head = mnp.tensordot(h, proj_weight, axes=1)
        # name = 'q' or 'k':
    ...
    def _create_mask(qlen, dtype, same_length=False):
        ...
        return ret.to(dtype=ms.float32)  # [qlen, qlen]
    ...
    def construct(self, inp_k, adjoin_matrix, perm_mask=None, target_mapping=None, inp_q=None, input_mask=None):
        ...
        adjoin_matrix = ops.transpose(adjoin_matrix, (1, 2, 0))   # [seq_len, seq_len, bsz]
        ...
        if self.attn_type == 'uni':
            attn_mask = self._create_mask(qlen, ms.int64, self.same_length)
        ...
        if attn_mask is not None:
            ...
            attn_mask = attn_mask.gt(0).to(dtype=ms.float32)

        if attn_mask is not None:
            # 负对角矩阵，(qlen, qlen)
            non_tgt_mask = -ops.eye(qlen, qlen, t=ms.float32)
            ...
            non_tgt_mask = (attn_mask +
                            non_tgt_mask[:, :, None, None]).gt(0).to(dtype=ms.float32)
        else:
            non_tgt_mask = None
...
class XLNetPreTrainModel(nn.Cell):
    ...
    def construct(self, inp_k, adjoin_matrix, label, perm_mask=None, target_mapping=None, inp_q=None):
        output,_, _ = self.xlnet_layer(inp_k, adjoin_matrix, perm_mask, target_mapping, inp_q)
...
class XLNetForClassificationModel(nn.Cell):
    ...
    def construct(self, inp_k, adjoin_matrix, label, cls_pos, perm_mask=None, target_mapping=None, inp_q=None):
        output, _, _ = self.xlnet_layer(inp_k, adjoin_matrix)
        output_split = ops.split(output, axis=0, output_num=output.shape[0])
        output_pieces = []
        # split batchs into single data
        for i in range(len(output_split)):
            # get single data
            one_feat = output_split[i].view(-1, output.shape[-1])
            # get single data cls index
            one_cls = cls_pos[i] - 1
            one_feat = one_feat[:one_cls, :]
            one_sum_feat = one_feat.sum(axis=0) / (one_cls + 1)
            # one_aver_feat = one_sum_feat / one_cls
            output_pieces.append(one_sum_feat)
        # concat single data as original batchs
        output_concat = ops.stack(output_pieces, axis=0)
        output = self.logits_proj(output_concat) # (bsz, 1)
...
class XLNetPredictModel(nn.Cell):
    ...
    def construct(self, inp_k, adjoin_matrix, cls_pos, perm_mask=None, target_mapping=None, inp_q=None):
        output, hidden_states, attentions = self.xlnet_layer(inp_k, adjoin_matrix)
        output_split = ops.split(output, axis=0, output_num=output.shape[0])
        output_pieces = []
        # split batchs into single data
        for i in range(len(output_split)):
